...
item_get_weight                     = 2700  # (item_get_weight, <destination_fixed_point>, <item_kind_no>),
                                            # Version 1.161+. Retrieves item weight as a fixed point value. The fixed point is the same as position's, so check carefully before use this shit.
...
change_screen_view_character          = 2046  # (change_screen_view_character),
                                              # Opens the character screen of another troop. Can only be used in dialogs and prsnt, cannot used in prsnt in mission.
...
scene_set_day_time                           = 1266  # (scene_set_day_time, <value>),
                                                     # Defines the time for the scene to force the engine to select a different skybox than the one dictated by current game time. Must be called within ti_before_mission_start trigger in module_mission_templates.py. Value should be in range 0..23. 0-23 means 24 hours of a day, 13 is noon.
...
set_current_color                           = 1950  # (set_current_color, x<red_value>, y<green_value>, z<blue_value>),
...
store_agent_hit_points                   = 1720  # (store_agent_hit_points, <destination>, <agent_id>, [absolute]),
                                                 # Retrieves current agent health. Optional last parameter determines whether actual health (absolute = 1) or relative percentile health (percentile = 0) is returned. Default is relative. For example, if your hp has only 15%, it will return 15 in the absolute = 0 mode. This percentage is caculated by troop's basic max hp, not new hp set by agent_set_max_hit_points.
agent_set_hit_points                     = 1721  # (agent_set_hit_points, <agent_id>, <value>,[absolute]),
                                                 # Sets new value for agent health. Optional last parameter determines whether the value is interpreted as actual health (absolute = 1) or relative percentile health (percentile = 0). Default is relative.
...
overlay_set_additional_render_height              =  952  # (overlay_set_additional_render_height, <overlay_id>, <height_adder>),
                                                          # Version 1.153+. Effects uncertain. 4research.The bigger the height_adder is, the fronter the overlay show.
