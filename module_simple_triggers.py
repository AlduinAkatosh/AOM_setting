   ...
   [          
     ...
     (else_try),
       ...
       (else_try),
         ...
         (start_encounter, "$auto_enter_town"),#休息、探索之后自动返回城镇，会重新触发script_game_encounter，务必注意
       (else_try),
...
#save lord from prison
  (1,#change form 0 to 1
	[
...
[
 ...
 #trigger 9
 (3, #Locate kingdom ladies
    [
      #change location for all ladies
       (assign, ":current_modula", "$g_kingdom_lady_modula"),
       (val_add, "$g_kingdom_lady_modula", 1),
       (try_begin),
          (eq, "$g_kingdom_lady_modula", 24),
          (assign, "$g_kingdom_lady_modula", 0),
       (try_end),

      (try_for_range, ":troop_id", kingdom_ladies_begin, kingdom_ladies_end),
        (store_mod, ":troop_modula", ":troop_id", 24),
...
#领主领地收入处理，以及伪阵营能提供的物资和部队的刷新
  ...
  (24*7,
   [
       (try_for_range, ":troop_no", active_npcs_begin, active_npcs_end),
         (troop_get_slot, ":cur_debt", ":troop_no", slot_troop_player_debt),#玩家欠领主钱的情况下利滚利1%
         (val_mul, ":cur_debt", 101),
         (val_div, ":cur_debt", 100),
         (troop_set_slot, ":troop_no", slot_troop_player_debt, ":cur_debt"),
         (call_script, "script_calculate_hero_weekly_net_income_and_add_to_wealth", ":troop_no"),#刷钱
       (try_end),
       ...
       (try_for_range, ":faction_no", "itm_faction_begin", "itm_faction_end"),#能提供的物资和部队的刷新
          (item_get_slot, ":count_no", ":faction_no", slot_faction_military_provide),
          (val_max, ":count_no", 0),
          (val_add, ":count_no", 1),
          (item_set_slot, ":faction_no", slot_faction_military_provide, ":count_no"),
          (item_get_slot, ":count_no", ":faction_no", slot_faction_goods_provide),
          (val_max, ":count_no", 0),
          (val_add, ":count_no", 1),
          (item_set_slot, ":faction_no", slot_faction_goods_provide, ":count_no"),
       (try_end),
    ]),
 ...
 #trigger 22
  ...
  (7,
   [
       (assign, ":current_modula", "$g_kingdom_wealth_modula"),
       (val_add, "$g_kingdom_wealth_modula", 1),
       (try_begin),
          (eq, "$g_kingdom_wealth_modula", 12),
          (assign, "$g_kingdom_wealth_modula", 0),
       (try_end),

#领主征兵
     (try_for_range, ":troop_no", active_npcs_begin, active_npcs_end),
        (store_mod, ":troop_modula", ":troop_no", 12),
        (eq, ":troop_modula", ":current_modula"),
       ...
       (party_slot_eq, ":cur_attached_party", slot_center_is_besieged_by, -1), #领主部队驻扎在城市里，且这个城没有被围攻
       ...
       (try_begin),
         (faction_slot_eq,  ":party_faction", slot_faction_marshall, ":troop_no"),#元帅多征兵一次
...
#据点征兵
     (try_for_range, ":center_no", walled_centers_begin, walled_centers_end),
        (store_mod, ":center_modula", ":center_no", 12),
        (eq, ":center_modula", ":current_modula"),

       (neg|party_slot_eq, ":center_no", slot_town_lord, "trp_player"), #玩家城镇不能自动征兵
       (party_slot_ge, ":center_no", slot_town_lord, 1), #城镇有领主       
       (party_slot_eq, ":center_no", slot_center_is_besieged_by, -1), #没被围攻
...
#diplomatic indices
 ...
 #trigger 29
  # Decide vassal ai
   (1,
    [
       (assign, ":current_modula", "$g_kingdom_vassal_modula"),
       (val_add, "$g_kingdom_vassal_modula", 1),
       (try_begin),
          (eq, "$g_kingdom_vassal_modula", 7),
          (assign, "$g_kingdom_vassal_modula", 0),
       (try_end),

       (call_script, "script_init_ai_calculation", ":current_modula"),
#       (call_script, "script_init_player_calculation"),

      #(call_script, "script_decide_kingdom_party_ais"),
      (try_for_range, ":troop_no", active_npcs_begin, active_npcs_end),
         (store_mod, ":troop_modula", ":troop_no", 7),
         (eq, ":troop_modula", ":current_modula"),
  ...
  (0.5,
   [
       (try_for_parties, ":party_no"),
         ...
         (party_get_attached_to, ":cur_attached_center", ":party_no"),       #haven't attach to any center
         (lt, ":cur_attached_center", 1),

         (party_get_slot, ":order_no", ":party_no", slot_party_orders_type),

         (try_begin),
             (store_faction_of_party, ":party_faction", ":party_no"),
             (eq, ":party_faction", "fac_player_supporters_faction"),           #player's auxiliary
             (eq, ":order_no", spai_holding_center),            #check order
             (party_get_slot, ":center_no", ":party_no",slot_party_orders_object),
             (party_is_in_town,":party_no",":center_no"),
             (assign, ":destination", ":center_no"),
             (party_attach_to_party, ":party_no", ":destination"),
         (else_try),
             (eq, ":order_no", spai_accompanying_army),                        #normal auxiliary
             (party_get_slot, ":lord_party", slot_party_ai_object, ":party_no"),
             (party_get_attached_to, ":center_no", ":lord_party"),                            #lord is in town
             (party_is_in_town, ":lord_party", ":center_no"), 
             (gt, ":center_no", 0),
             (store_distance_to_party_from_party, ":distance", ":center_no", ":party_no"),      #around the town
             (le, ":distance", 1),
             (assign, ":destination", ":center_no"),
             (party_attach_to_party, ":party_no", ":destination"),
         ...
         (try_begin),
           (gt, ":destination", 0),
           (this_or_next|party_slot_eq, ":destination", slot_party_type, spt_town),     #put soliders to town or castle
           (party_slot_eq, ":destination", slot_party_type, spt_castle),
           (store_faction_of_party, ":party_faction_no", ":party_no"),
           (store_faction_of_party, ":destination_faction_no", ":destination"),
           (eq, ":party_faction_no", ":destination_faction_no"),
           (party_get_num_prisoner_stacks, ":num_stacks", ":party_no"),
           (gt, ":num_stacks", 0),
           (assign, "$g_move_heroes", 1),
           (call_script, "script_party_prisoners_add_party_prisoners", ":destination", ":party_no"),#Moving prisoners to the center
           (assign, "$g_move_heroes", 1),
 ...
 #Update how good a target player is for bandits
  ...
  (72,
   [
     ...
     #NPC changes begin
     # removes   (call_script, "script_update_companion_candidates_in_taverns"),#in trigger
     #NPC changes end
...
#UPGRADE
  ...
  (1, [(neg|map_free),
       ...
       (else_try),
         (eq, "$g_player_reading_book", "itm_book_necromancer_low"),
         (call_script, "script_get_an_unlearned_undead"),
         (call_script, "script_learn_certain_undead", reg1),
         (str_store_string, s2, "@Learn_undead"),
         (try_begin),
            (gt, "$book_necromancer_low_read", 1),
            (assign, "$book_necromancer_low_read", 1),
         (else_try),
            (val_add, "$book_necromancer_low_read", 1),
         (try_end),
       (try_end),
...
# Report to army quest 
  (2,
   [
     ...
     (quest_set_slot, "qst_report_to_army", slot_quest_dont_give_again_period,  ),     
...
#zhiyuan
      (1,
        [
(eq, 0, 1),#need further research
          (try_for_parties, ":party_no"),
            (party_is_active, ":party_no"),
            (store_faction_of_party, ":party_faction", ":party_no"),
            (this_or_next|eq, ":party_faction", "fac_outlaws_bandit"),
            (eq, ":party_faction", "fac_outlaws_deserters"),
            (party_get_battle_opponent, ":opponent", ":party_no"),
            (try_begin),
              ...
              (try_for_parties, ":party_no_2"),
                ...
                (this_or_next|eq, ":party_faction_2", "fac_outlaws_bandit"),
                (eq, ":party_faction_2", "fac_outlaws_deserters"),
...
#UPGRADE
#get auxiliary behavior from orders
    (0.1,
      [
        (try_for_parties, ":party_no"),
          (party_slot_eq, ":party_no", slot_party_type, spt_attendant_party),   #is auxiliary 
          (party_get_slot, ":order_type", ":party_no", slot_party_orders_type),
          (party_get_slot,  ":target_no", ":party_no", slot_party_orders_object),
          (try_begin),
               (eq, ":order_type", spai_undefined),                     #stay here
               (party_set_ai_behavior, ":party_no",ai_bhvr_hold), 
               (party_set_courage, ":party_no", 8),
               (party_set_aggressiveness, ":party_no", 0),
               (party_set_ai_initiative,":party_no",20),
          (else_try),
               (eq, ":order_type", spai_accompanying_army),    #guard a party
               (party_set_ai_behavior, ":party_no", ai_bhvr_escort_party), 
               (party_set_ai_object, ":party_no", ":target_no"),
               (party_set_courage, ":party_no", 8),
               (party_set_aggressiveness, ":party_no", 0),
               (party_set_ai_initiative,":party_no",20),
          (else_try),
               (eq, ":order_type", spai_patrolling_around_center),
               (neg|is_between, ":target_no", centers_begin, centers_end),   #patrol around a party
               (party_set_ai_behavior, ":party_no", ai_bhvr_patrol_party), 
               (party_set_ai_object, ":party_no", ":target_no"),        #didin't set initiative, courage and aggressive because these are setted in the dialog.
          (else_try),
               (eq, ":order_type", spai_patrolling_around_center),   #patrol around a center
               (party_set_ai_behavior, ":party_no", ai_bhvr_patrol_location), 
               (party_set_ai_object, ":party_no", ":target_no"),
               (party_set_courage, ":party_no", 8),
               (party_set_aggressiveness, ":party_no", 10),
               (party_set_ai_initiative,":party_no",100),
               (party_set_ai_patrol_radius,":party_no",3),
          (else_try),
               (eq, ":order_type", spai_retreating_to_center),                   #hide in a town
               (party_set_ai_behavior, ":party_no", ai_bhvr_travel_to_party), 
               (party_set_ai_object, ":party_no", ":target_no"),
               (party_set_courage, ":party_no", 2),
               (party_set_aggressiveness, ":party_no", 0),
               (party_set_ai_initiative,":party_no",40),
          (else_try),
               (eq, ":order_type", spai_holding_center),                           #join a town's garrison
               (party_set_ai_behavior, ":party_no", ai_bhvr_travel_to_party), 
               (party_set_ai_object, ":party_no", ":target_no"),
               (party_set_courage, ":party_no", 6),
               (party_set_aggressiveness, ":party_no", 4),
               (party_set_ai_initiative,":party_no",80),
          (else_try),
               (eq, ":order_type", spai_besieging_center),                           #attack a party
    ...
    (1,
      [
        (try_begin),
           ...
           (set_spawn_radius, 1),
           (spawn_around_party, "p_main_party", "pt_leaded_looters"),#活祭猎人
           (assign, "$g_plot_enemy", reg0),
           (start_encounter, "$g_plot_enemy"),
        (else_try),
           (map_free),
           (this_or_next|eq, "$third_death_quest_phase", 9),#第一次从旧加尔村返回
           (this_or_next|eq, "$third_death_quest_phase", 11),#和护枪官对话出来
           (this_or_next|eq, "$third_death_quest_phase", 15),#第二次返回
           (this_or_next|eq, "$third_death_quest_phase", 17),#和吟游诗人对话出来
           (eq, "$third_death_quest_phase", 19),#和吟游诗人对话出来
        ...
        (else_try),
           ...
           (start_map_conversation, "trp_npc19"),#梅薇丝
        (try_end),

        (val_sub, "$exporation_time", 1),#探索的计时器
        (val_max, "$exporation_time", 0),
     ]),
#任务专用触发器（按天计）
    (24,
      [
        (try_begin),
           (is_between, "$third_death_quest_phase", 23, 25),#23未归还肖像枪，24已归还
           (val_add, "$portrait_lance_corrosion", 1),#侵蚀度
           (try_begin),
              (check_quest_active, "qst_third_death"),#支线剧情：第三个死
              (eq, "$third_death_quest_phase", 23),
              (eq, "$portrait_lance_corrosion", 32),#暴毙
              (jump_to_menu, "mnu_death_end"),
           (else_try),
              (neg|check_quest_active, "qst_third_death"),#任务完成
              (eq, "$portrait_lance_corrosion", 40),#发信息称可以接收侠义骑士了
              (display_message, "@护 枪 官 给 你 发 来 信 件 ， 称 他 们 近 来 的 宣 传 工 作 取 得 了 成 效 ， 已 经 有 一 批 侠 义 骑 士 慕 名 而 来 了 。 你 可 以 择 日 去 看 看 。 "),
           (try_end),
        (try_end),
     ]),
]
