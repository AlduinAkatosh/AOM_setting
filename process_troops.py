...
def save_python_header():
  ...
  file.close()
  ## CC
  file = open("./ID_attributes.py","w")
  file.write("from header_common import *\n\n")
  for i in xrange(3, int(0xFF)+1):
    file.write("str_%d = bignum | %s\n"%(i, hex(i)))
  for i in xrange(3, int(0xFF)+1):
    file.write("agi_%d = bignum | %s\n"%(i, hex(i<<8)))
  for i in xrange(3, int(0xFF)+1):
    file.write("int_%d = bignum | %s\n"%(i, hex(i<<16)))
  for i in xrange(3, int(0xFF)+1):
    file.write("cha_%d = bignum | %s\n"%(i, hex(i<<24)))
  file.close()
  ## CC
